...
import {
  ...
  compactInlineFilterFormStyle,
  filterCellFlex,
  TABLE_SCROLL_Y_STACKED,
} from "../utils/tablePageLayout";

const UserManagement = () => {
  const loginUserId = getUser()?.id;
  ...
  const [permanentDeleteModalVisible, setPermanentDeleteModalVisible] =
    useState(false);
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      if (listQuery.roleId != null) params.roleId = listQuery.roleId;
      if (listQuery.permissionId != null)
        params.permissionId = listQuery.permissionId;
      if (
        listQuery.status !== undefined &&
        listQuery.status !== null &&
        listQuery.status !== ""
      ) {
        params.status = listQuery.status;
      }
  ...
  }, [pagination.current, pagination.pageSize, listQuery]);
  ...
  useEffect(() => {
    ...
    deletedUsersPagination.current,
    deletedUsersPagination.pageSize,
    recycleBinModalVisible,
  ]);
  ...
  const permissionSelectOptions = useMemo(() => {
    ...
    list.sort((a, b) =>
      (a.permissionName || "").localeCompare(b.permissionName || "", "zh-CN"),
    );
    return list.map((p) => ({
      value: p.id,
  ...
  const handleSavePermissions = async () => {
    try {
      ...
      message.error(
        error.response?.data?.msg || error.message || "权限更新失败",
      );
    }
  };
  ...
  const handleCloseTab = (targetKey, e) => {
    ...
    if (targetKey === activeTabKey) {
      ...
      if (newVisibleTabs.length > 0) {
        ...
        const nextIndex =
          currentIndex < newVisibleTabs.length
            ? currentIndex
            : currentIndex - 1;
  ...
  const handleStatusChange = async (id, status) => {
    ...
    try {
      ...
      const errorMsg =
        error.response?.data?.msg || error.message || "状态更新失败";
  ...
  const handleConfirmPermanentDelete = async () => {
    try {
      ...
      message.error(
        error.response?.data?.msg || error.message || "永久删除失败",
      );
    }
  };
      ...
      render: (_, record) => (
        <Space>
          ...
          {hasPermission(PERMISSIONS.USER_UPDATE) && (
            <Tooltip
              title={
                record.id === loginUserId && record.status === 1
                  ? "不能禁用当前登录账号"
                  : record.status === 1
                    ? "禁用"
                    : "启用"
            ...
            >
              <Button
                ...
                disabled={record.id === loginUserId && record.status === 1}
                icon={
                  record.status === 1 ? (
                    <CloseCircleOutlined />
                  ) : (
  ...
  return (
    <div style={pageRootStyle}>
      <div style={toolbarSectionStackedStyle}>
        <h2 style={toolbarPageTitleStyle}>用户管理</h2>
        <div style={compactFilterRowFullWidthStyle}>
          <Card size="small" style={{ flex: 1, minWidth: 0, marginBottom: 0 }}>
            <Form
              form={filterForm}
              layout="inline"
              colon={false}
              style={compactInlineFilterFormStyle}
            >
              <Form.Item
                name="keyword"
                label="关键词"
                ...
                style={{
                  marginBottom: 0,
                  marginInlineEnd: 0,
                  ...filterCellFlex("1.2 1 120px", 120, 320),
                }}
              >
                <Input
        ...
        {currentUserPermissions && (
          <div>
            <Card size="small" style={{ marginBottom: 16 }}>
              ...
              <div>
                <strong>角色：</strong>
                {roles.find((r) => r.id === currentUserPermissions.roleId)
                  ?.roleName || "-"}
              </div>
            </Card>
                        ...
                        >
                          <Space direction="vertical" style={{ width: "100%" }}>
                            {categoryPermissions.map((permission) => {
                              const isRolePermission = rolePermissions.some(
                ...
                {Object.keys(groupedPermissions)
                  ...
                  .map((category) => (
                    <Button
                      ...
                      size="small"
                      type={
                        visibleTabs.includes(category) ? "default" : "dashed"
                      }
            ...
            <div style={{ marginTop: 16, color: "#999", fontSize: 12 }}>
              ...
              <ul style={{ margin: 0, paddingLeft: 20 }}>
                <li>
                  点击标签右侧的 ×
                  可关闭该分类，关闭后该分类下的所有权限将被移除
                </li>
                <li>
                  绿色标签表示来自角色的权限，取消勾选后将从用户权限中移除
                </li>
                <li>蓝色标签表示用户直接拥有的权限</li>
                <li>用户最终拥有的权限 = 角色权限 + 用户直接权限（去重）</li>
                <li>
                  用户不需要拥有所属角色的所有权限，可以移除不需要的角色权限
                </li>
              </ul>
            </div>
          </div>
        ...
        <Table
          columns={[
            ...
            },
            {
              ...
              width: 200,
              render: (_, record) => {
                const isSelf = record.id === loginUserId;
                return (
                  <Space>
                    <Tooltip title={isSelf ? "不能恢复当前登录账号" : "恢复"}>
                      <Button
                        type="link"
                        icon={<UndoOutlined />}
                        disabled={isSelf}
                    ...
                    <Tooltip title={isSelf ? "不能永久删除当前登录账号" : ""}>
                      <span>
                        <Popconfirm
                          title="确定要永久删除吗？"
      ...
      >
        <div style={{ marginBottom: 16 }}>
          ...
          <p>
            请输入 <strong style={{ color: "#ff4d4f" }}>DELETE</strong>{" "}
            以确认此操作：
          </p>
        </div>
