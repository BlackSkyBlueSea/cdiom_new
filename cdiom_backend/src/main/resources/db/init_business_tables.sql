...
CREATE TABLE IF NOT EXISTS outbound_apply (
    ...
    status VARCHAR(20) DEFAULT 'PENDING' COMMENT 'PENDING-待审批/PENDING_SECOND-待第二审批/APPROVED-已通过/REJECTED-已驳回/OUTBOUND-已出库/CANCELLED-已取消',
    approver_id BIGINT DEFAULT NULL COMMENT '审批人ID',
    second_approver_id BIGINT DEFAULT NULL COMMENT '第二审批人ID（特殊药品）',
    approve_time DATETIME DEFAULT NULL COMMENT '终批通过时间',
    first_approve_time DATETIME DEFAULT NULL COMMENT '第一审批通过时间（特殊药品待第二审批时）',
    reject_reason VARCHAR(500) DEFAULT NULL COMMENT '驳回理由',
    reject_operator_id BIGINT DEFAULT NULL COMMENT '驳回操作人（待第二审批时第二人驳回，保留 approver_id 为第一审批人）',
