...
#### 17. `add_user_permission_system.sql` ⭐ **新增**
- **用途**：细粒度权限系统升级脚本
- **说明**：支持用户直接拥有权限，实现更细粒度的权限控制
- **包含内容**：
  - 创建用户权限关联表（`sys_user_permission`）
  - 添加细粒度出库相关权限（outbound:view, outbound:apply, outbound:approve等）
  - 添加细粒度入库相关权限（inbound:view, inbound:create, inbound:approve等）
  - 添加细粒度库存管理权限（inventory:view, inventory:adjust, inventory:adjust:approve）
  - 为各角色分配相应的细粒度权限
  - 包含权限验证查询语句
- **执行方式**：
  ```bash
  mysql -u root -p < add_user_permission_system.sql
  ```
- **注意事项**：
  - 该脚本会创建新的权限表，支持用户直接拥有权限
  - 执行前建议备份数据库
  - 该脚本会添加大量新的权限代码，请确保权限代码不与现有权限冲突

#### 18. `update_role_permissions_complete.sql` ⭐ **新增**
- **用途**：完整角色权限配置更新脚本
- **说明**：更新所有角色的权限配置，包含细粒度权限分配
- **包含内容**：
  - 为系统管理员分配所有权限
  - 为仓库管理员分配库存、入库、出库相关权限
  - 为采购专员分配采购订单、供应商管理权限
  - 为医护人员分配出库申领权限
  - 为供应商分配订单查看权限
  - 包含权限验证查询语句
- **执行方式**：
  ```bash
  mysql -u root -p < update_role_permissions_complete.sql
  ```
- **注意事项**：
  - 该脚本会更新所有角色的权限配置
  - 执行前建议备份数据库
  - 建议在 `add_user_permission_system.sql` 之后执行
...
### Q9: 如何启用细粒度权限系统？
**A**: 按顺序执行以下脚本：
1. 执行 `add_user_permission_system.sql` 创建用户权限关联表和细粒度权限
2. 执行 `update_role_permissions_complete.sql` 更新角色权限配置
3. 验证权限配置是否正确

### Q10: 用户权限关联表的作用是什么？
**A**: `sys_user_permission` 表支持用户直接拥有权限，实现更细粒度的权限控制。除了通过角色获得权限外，用户还可以直接拥有特定权限，适用于特殊场景的权限分配。
