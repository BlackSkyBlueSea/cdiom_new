...
#### UPDATE_LOG_20260309_Outbound_Dashboard.md
- **内容**：近期开发记录（2026-03-08～03-09）
- **包含**：
  - 出库申请与审批业务流概览
  - 代码与功能改动清单（后端/前端/权限、JWT 优先级、审批前库存校验等）
  - 与 CHANGELOG、API、测试报告等文档的对应关系
  - 新增/变更接口速查

#### UPDATE_LOG_20260328.md
- **内容**：近期开发记录（2026-03-28 前后工作区变更汇总）
- **包含**：
  - `SystemConfigUtil` / `SysConfigKeys`：近效期、JWT 时长、日志保留等与 `sys_config` 贯通
  - JWT 签发与定时日志清理、管理员联系信息接口、用户管理「禁止操作当前登录账号」等
