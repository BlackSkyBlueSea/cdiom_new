...
**与业务映射的常见键名（示例，具体以初始化数据与参数管理界面为准）**：
- `expiry_warning_days` / `expiry_critical_days`：近效期黄色/红色预警阈值（天），仪表盘、库存、入库效期校验等读取运行时值
- `jwt_expiration`：JWT 有效期（毫秒），合法时优先生效于 `application.yml` 的 `jwt.expiration`
- `log_retention_years`：登录日志与操作日志保留年限；后端定时任务可按该值清理更早的记录（部署前须符合合规与审计要求）
...
**说明**：操作日志表通常**不在业务界面提供删除**；系统可通过配置的 **`log_retention_years`** 由定时任务**物理清理**早于保留区间的记录。实际保留策略须满足本单位合规与审计要求。
...
**说明**：登录日志通常**不在业务界面提供删除**；可与操作日志一样，由定时任务按 **`log_retention_years`** 清理更早记录，具体须符合合规与审计要求。
...
- 操作日志与登录日志不在界面随意删除；保留年限可由 **`log_retention_years`** 配置，并由定时任务清理过期数据（须满足合规）
...
2. **操作/登录日志**：界面不提供随意删除；可按 **`log_retention_years`** 自动清理历史数据，部署前须确认符合审计与法规要求
