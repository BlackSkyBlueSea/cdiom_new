...
#### 2.9.1 参数配置界面手工用例（CFG-01 / CFG-02）

以下与汇总表 **CFG-01（近效期阈值修改）**、**CFG-02（红色预警阈值）** 对应，便于按步骤执行与记录；需具备 `config:manage` 权限。

**CFG-01 参数修改验证（近效期预警阈值，`expiry_warning_days`）**

| 步骤 | 测试步骤 | 预期结果 |
|:----:|----------|----------|
| 1 | 使用系统管理员登录，进入 **参数配置** 页面 | 页面展示配置列表，含 **当前业务生效** 列 |
| 2 | 定位「近效期预警阈值（天）」或键名 `expiry_warning_days`，将参数值改为 **150**，点击保存 | 提示保存成功 |
| 3 | 刷新页面或点击「刷新生效参数」，查看 **当前业务生效** 列与「参数值」列对应行 | 「参数值」为 **150**；`expiry_warning_days` 行「当前业务生效」为 **150 天**；接口 `GET /api/v1/configs/runtime-effective` 中 `expiryWarningDays` 为 **150** |
| 4 | （可选）切换仓库管理员账号，打开 **仓库管理员工作台**，查看近效期黄色预警相关展示 | 标题或统计与当前配置一致（黄色区间为「红色阈值+1～预警阈值」天；须保证 **预警阈值 &gt; 红色阈值**，否则可能显示「按系统预警设置」） |

**CFG-02 红色预警阈值修改与映射验证（`expiry_critical_days`）**

| 步骤 | 测试步骤 | 预期结果 |
|:----:|----------|----------|
| 1 | 使用系统管理员登录，进入 **参数配置** 页面 | 列表与 **当前业务生效** 列加载正常 |
| 2 | 定位「红色预警阈值（天）」或键名 `expiry_critical_days`，将参数值改为 **60**（**须小于**当前 `expiry_warning_days`，例如上一步已为 150 则满足），点击保存 | 提示保存成功 |
| 3 | 点击「刷新生效参数」，或调用 `GET /api/v1/configs/runtime-effective` | 返回体中 `expiryCriticalDays` 为 **60** |
| 4 | 切换 **仓库管理员**，打开 **仓库管理员工作台**，查看「近效期预警（≤…天）」红色卡片及下方预警提示文案 | 标题含 **≤60 天**（或与运行时一致）；红色预警批次数与后端按「剩余效期 ≤60 天」统计一致 |
| 5 | 进入 **库存管理**，查看「预警状态」列：剩余天数 ≤60 的批次应呈现红色预警样式 | 与 `expiry_critical_days` 规则一致 |
| 6 | （可选）在 **入库** 流程中对效期进行校验，效期落在红色区间内应出现 **FORCE** 或严重预警类提示（与入库效期校验逻辑一致） | 行为符合当前配置 |
| 7 | 测试结束后将 `expiry_critical_days`、`expiry_warning_days` 恢复为环境常用值（如 90 / 180）并保存 | 保存成功，表中「当前业务生效」列与业务页面恢复一致 |

**说明**：若将红色阈值改得 **≥** 近效期预警阈值，黄色区间可能为空，界面可能显示「按系统预警设置」；测试时应保持 **预警阈值（天）&gt; 红色阈值（天）**。
