...
- 用户列表查询（分页、关键字搜索、角色筛选、状态筛选、**按权限 ID 筛选** `permissionId`）
- 用户新增、编辑、删除（逻辑删除）
- 用户状态管理（启用/禁用）
- 用户解锁功能
- 用户权限管理（用户直接权限关联）
...
- 参数配置新增、编辑、删除（逻辑删除）
- 根据键名查询参数配置
- 运行时生效参数查询：`GET /api/v1/configs/runtime-effective`（近效期阈值、JWT 时长、日志保留年限、登录锁定参数等）
...
- 药品信息列表查询（分页、关键字搜索、特殊药品筛选、**可选排序** `sortField`/`sortOrder`、按供应商筛选）
- 药品信息新增、编辑、删除（逻辑删除）
- **回收站**：分页查询已逻辑删除的药品、**恢复**指定药品（`GET .../deleted`、`PUT .../{id}/restore`，需 `drug:manage`）
...
- 采购订单列表查询（分页、关键字搜索、状态筛选、供应商筛选；列表/明细中带 **供应商名称**、明细中带 **药品名称与规格** 等展示字段）
- 采购订单新增、编辑、删除
...
- 入库记录列表查询（分页、多条件筛选；记录中带 **药品名称 `drugName`** 便于展示）
- **到货批次**：登记采购到货批次头（`POST /inbound/receipt-batch`），从订单入库时可复用 `receiptBatchId`
- 创建入库记录（支持从订单入库和临时入库；支持 **同一批号合格/不合格拆分** `from-order/split`）
- **不合格处置**：处置意向下拉与 `GET /inbound/disposition-options` 统一口径
...
- **近效期明细**：`GET /dashboard/warehouse/near-expiry-details?level=yellow|red`，与卡片预警区间一致
...
- 获取当前用户权限列表
- 用户登出
- 系统管理员联系信息：`GET /api/v1/auth/admin-contact`（已登录用户可调用，用于个人中心等）
...
- 用户名/手机号登录
- 密码加密传输
- Token **始终写入 `sessionStorage`**；单用户模式下同时写入 Cookie `cdiom_token`（约 8 小时）；多用户/多标签场景下依赖 `sessionStorage` 避免身份串用（与 `auth.js`、`request.js` 一致）
- 支持多用户登录模式（`sessionStorage` 标记与路由参数）
...
- 仓库管理员仪表盘（近效期预警、待办任务、出入库统计；**近效期明细**与预警卡片区间一致）
...
#### ✅ 药品管理页面
- 药品信息列表展示（分页、关键字搜索、特殊药品筛选、**列排序**、按供应商筛选）
- 药品信息新增、编辑、删除；**回收站**（已删除药品列表与恢复）
- 四种新增药品方式：
  1. 全部手动输入
...
#### ✅ 供应商管理页面增强
- 供应商列表展示、新增、编辑、删除
- 供应商-药品关联管理（添加、删除、更新单价）；独立路由页 **`SupplierDrugManage.jsx`**（`/supplier-drugs`）集中维护关联关系
- 根据供应商动态获取关联药品列表
...
#### ✅ 库存管理页面
- 库存列表展示（分页、多条件筛选；列表数据含 **药品名称** 等展示字段）
...
- ✅ 条形码服务：100%（订单条形码生成）
- ✅ IP定位服务：100%（登录地点识别）
- ✅ 数据导出：100%（Excel导出：药品列表、库存列表、采购订单列表，已在后端接口与前端页面中联调验证通过）
- ⏳ 扫码枪适配：0%（HID键盘模式待实现）
...
- ✅ 数据库设计：100%（`init_simple.sql` 20 张基础表 + 按 README 执行扩展脚本；详见 [Database_Design.md](./Database_Design.md)）
